...
void init_assistnow();
bool download_online_blob(time_t epoch_time);
bool download_offline_blob(time_t epoch_time);
...
void setup()
{
  ...
  {
    File ownerFile = SDFS.open(OWNER_FILENAME, "w");
    ownerFile.print(OWNER_CONTENT);
    ownerFile.close();
...
bool connect_wifi()
{
  ...
  delay(1000);
  log_printfln("NTP updated:     %lu", timeClient.getEpochTime());
...
void loop()
{
  while (uart_gps.available() > 0)
  {
    ...
    if (!assistnow_initialized && gps.date.isValid() && gps.time.isValid())
    {
      init_assistnow();
    }
...
void persist_location_record()
{
  ...
  double distance_travelled = gps.distanceBetween(old_lat, old_lng, new_lat, new_lng);
  ...
  char record[80];
  sprintf(record, TIMESTAMP_FORMAT ";%.6f;%.6f;%.2f;%.2f;%d",
          TIMESTAMP_ARGS,
          new_lat, new_lng, gps.altitude.meters(), gps.speed.mps(), gps.satellites.value());
...
void upload_gps_file()
{
  ...
  if (http_code == HTTP_CODE_OK)
  {
    ...
    SDFS.remove(GPS_FILENAME);
    log_println("GPS file deleted from SD card.");
    init_gps_file();
  }
  else
  {
...
void cleanup_outdated_assistnow_blobs(time_t epoch_time)
{
  ...
  while (dir.next())
  {
    if (dir.fileName().startsWith("ASSISTNOW-") && dir.fileName().endsWith(".bin"))
    {
      ...
      {
        log_printfln("AssistNow: keeping still valid blob: %s", dir.fileName().c_str());
      }
    }
  }
}

String find_valid_assistnow_blob(time_t epoch_time)
{
  String best_match = "";
  time_t best_epoch = 0;
...
void init_assistnow()
{
  ...
  log_printfln("GPS module time: %lld - " TIMESTAMP_FORMAT, epoch_time, TIMESTAMP_ARGS);

  cleanup_outdated_assistnow_blobs(epoch_time);

  // download OFFLINE data first
  download_offline_blob(epoch_time);
  // download ONLINE data and immediately upload it as the most recent blob
  download_online_blob(epoch_time);
  String filename = find_valid_assistnow_blob(epoch_time);
